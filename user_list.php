        ...
        if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
            ...
            if (isset($_SESSION['permissions']) && $_SESSION['permissions'] == 'super') {
              echo '<form action="delete-query_user.php" method="POST" onsubmit="return confirm(\'Are you sure you want to delete this item?\');">
                      <input type="hidden" name="id" value="' . $row["user_id"] . '">
                      <input type="submit" id="admin_buttons" name="delete" value="Delete" />
                    </form>
                    <form action="modify_user.php?id=' . $row["user_id"] . '" method="POST">
                      <input type=\'hidden\' name=\'id\' value=\'' . $row["user_id"] . '\'>
